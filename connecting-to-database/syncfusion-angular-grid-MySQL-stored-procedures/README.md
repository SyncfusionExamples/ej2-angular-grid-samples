...
- **Syncfusion Angular Grid**: Built-in search, filter, sort, and paging capabilities with UrlAdaptor
...
1. **Clone the repository**
   ...
   cd syncfusion-angular-grid-MySQL-stored-procedures
   ```
...
5. **Run the application**
   ...
   In a separate terminal, start the Angular client (from grid_mysql.client folder):
   ```bash
   ng serve
   ```
...
6. **Open the application**
   ...
   Navigate to the URL displayed in the Angular terminal (typically `http://localhost:4200`).
...
| File/Folder | Purpose |
|-------------|---------|
| `/src/app/app.component.ts` | Angular Grid component with UrlAdaptor, toolbar, and data operations |
| `/src/app/app.component.html` | Angular Grid template markup |
| `/src/app/custom-adaptor.ts` | Custom data adaptor for advanced data operations (optional) |
| `/src/styles.css` | Syncfusion theme imports and styling |
...
### CORS Issues
- Ensure the ASP.NET Core server CORS policy allows the Angular client origin
...
- Check that CSS imports in `styles.css` match your installed package versions
- Verify Angular CLI version is compatible with the project
- Check that all required Angular modules are imported in component files
...
Detailed, step-by-step directions are available in the [user guide](./mysql-sotred-procedure.md).