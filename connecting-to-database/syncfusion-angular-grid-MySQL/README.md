# Angular Grid with MySQL and LINQ2DB

## Project Overview

This repository demonstrates a production-ready pattern for binding **MySQL Server** data to **Syncfusion Angular Grid** using **LINQ2DB lightweight ORM**. The sample application provides complete CRUD (Create, Read, Update, Delete) operations, filtering, sorting, paging, and batch updates. The implementation follows industry best practices using ASP.NET Core Web API, LINQ2DB for type-safe database queries, and Angular services for seamless grid-to-server communication.

## Key Features

- **MySQL–LINQ2DB Integration**: Type-safe LINQ queries with minimal overhead compared to full ORMs like Entity Framework
- **Syncfusion Angular Grid**: Built-in search, filter, sort, paging, and toolbar capabilities
...
- **Data Service**: Full control over grid data operations (read, search, filter, sort, page)
...
|-----------|---------|---------|
| Visual Studio 2022 | 17.0 or later | Development IDE with Angular and ASP.NET Core workload |
...
1. **Clone the repository**
   ...
   cd syncfusion-angular-grid-MySQL
   ```
...
5. **Install client dependencies and run the Angular app**
...
| `/src/app/app.component.ts` | Main Angular component with Grid configuration and column definitions |
| `/src/app/services/transaction.service.ts` | Service for API communication |
| `/src/app/models/transaction.model.ts` | TypeScript model for Transaction data |
| `/src/app/custom-adaptor.ts` | Custom data adaptor extending UrlAdaptor for handling grid operations |
| `/src/styles.css` | Global CSS styles including Syncfusion theme imports |
| `/src/main.ts` | Angular application entry point |
...
1. **Client Request**: Angular Grid sends HTTP POST requests to `/api/grid/*` endpoints
...
6. **Response**: Data flows back through DataConnection → controller → HTTP response → Angular Grid

### Data Service

The `TransactionService` communicates with the backend API and handles:
...
- Ensure the `TransactionService` is correctly configured in the Angular component
...
### Port Already in Use
- If port 5283 or 4200 is in use, modify the port in:
  - Server: `Grid_MySQL.Server/Properties/launchSettings.json` (change `applicationUrl`)
  - Client: Update the API URL in `src/environments/environment.ts`

## Reference
The [user guide](https://ej2.syncfusion.com/angular/documentation/grid/connecting-to-database/mysql-server) provides detailed directions in a clear, step-by-step format.
...
2. **Copy the GitHub URL**
    ...
    - Example : https://github.com/SyncfusionExamples/ej2-angular-grid-samples/tree/master/connecting-to-database/syncfusion-angular-grid-MySQL
