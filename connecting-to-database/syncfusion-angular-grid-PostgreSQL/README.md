# Angular Grid with PostgreSQL and Entity Framework Core

## Project Overview

This repository demonstrates a production-ready pattern for binding **PostgreSQL** data to **Syncfusion Angular Grid** using **Entity Framework Core (EF Core)**. The sample application provides complete CRUD (Create, Read, Update, Delete) operations, filtering, sorting, paging, and batch updates. The implementation follows industry best practices using ASP.NET Core Web API, DbContext, Entity Framework Core with Npgsql provider, and Angular services for seamless grid-to-server communication.
...
| `src/app/app.ts` | Main Angular component with Grid configuration and DataManager |
| `src/app/app.html` | Grid template with columns and CRUD toolbar |
| `src/app/custom-adaptor.ts` | CustomAdaptor for API communication and data operations |
...
1. **Client Request**: Angular Grid sends HTTP POST requests to `/api/PurchaseOrder/*` endpoints
...
The **CustomAdaptor** intercepts data operations and handles:
...
| `Entry().State = Modified` | Marks entities for update |
| `Remove()` | Deletes entities from the database |
| `SaveChanges()` | Persists all changes to PostgreSQL |
| `UseNpgsql()` | Configures Npgsql as the database provider |
...
### Version Conflicts
- Ensure Entity Framework Core, Npgsql provider, and Angular packages are compatible versions
...
- Verify all packages are compatible with .NET 10.0 and Node.js 18.x+
...
2. **Copy the GitHub URL**
   ...
   - Example: https://github.com/SyncfusionExamples/ej2-angular-grid-samples/tree/master/connecting-to-database/syncfusion-angular-grid-postgresql
