# Angular Grid with SQLite and Entity Framework Core

## Project Overview

This repository demonstrates a production-ready pattern for binding **SQLite** data to **Syncfusion Angular Grid** using **Entity Framework Core (EF Core)**. SQLite is a lightweight, serverless, embedded database that is ideal for mobile applications, desktop applications, and small-to-medium scale web applications. The sample application provides complete CRUD (Create, Read, Update, Delete) operations, filtering, sorting, paging, and batch updates with a clean separation between the Angular frontend and ASP.NET Core backend.
...
|-----------|---------|---------|
| Visual Studio 2022 | 17.0 or later | Development IDE with ASP.NET Core and Angular workload |
...
1. **Clone the repository**
   ...
   cd syncfusion-angular-grid-SQLite
   ```
...
| `/src/app/app.ts` | Main Angular component with Grid configuration and column templates |
| `/src/app/services/asset.service.ts` | Service for API communication |
| `/src/app/models/asset.model.ts` | TypeScript model for Asset data |
...
1. **Client Request**: Angular Grid sends HTTP POST requests to `/api/asset/*` endpoints
...
The `AssetService` communicates with the backend API and handles:
...
- Ensure the `AssetService` is correctly configured in the Angular component
...
## Reference
The [user guide](https://ej2.syncfusion.com/angular/documentation/grid/connecting-to-database/sqlite-server) provides detailed directions in a clear, step-by-step format.
