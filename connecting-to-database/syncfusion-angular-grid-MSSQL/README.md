# Angular Grid with SQL Server and ADO.NET SqlClient

## Project Overview

This repository demonstrates a production-ready pattern for binding **MSSQL Server** data to **Syncfusion Angular Grid** using **ADO.NET SqlClient**. The sample application provides complete CRUD (Create, Read, Update, Delete) operations, filtering, sorting, paging, and batch updates. The implementation follows industry best practices using ASP.NET Core Web API, repository pattern, and Angular services for seamless grid-to-server communication.
...
1. **Clone the repository**
   ...
   cd syncfusion-angular-grid-MSSQL
   ```
...
| `/src/app/app.component.ts` | Main Angular component with Grid configuration and column templates |
| `/src/app/services/ticket.service.ts` | Service for API communication |
| `/src/app/models/ticket.model.ts` | TypeScript model for Ticket data |
...
1. **Client Request**: Angular Grid sends HTTP POST requests to `/api/tickets` endpoints
...
6. **Response**: Data flows back through repository → controller → HTTP response → Angular Grid

### Data Service

The `TicketService` communicates with the backend API and handles:
...
- Ensure the TicketService is correctly configured in the Angular component
- Verify the custom adaptor is properly wired in the component
...
## Reference
The [user guide](https://ej2.syncfusion.com/angular/documentation/grid/connecting-to-database/microsoft-sql-server) provides detailed directions in a clear, step-by-step format.
...
2. **Copy the GitHub URL**
    ...
    - Example : https://github.com/SyncfusionExamples/ej2-angular-grid-samples/tree/master/connecting-to-database/syncfusion-angular-grid-MSSQL
