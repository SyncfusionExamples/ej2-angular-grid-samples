# Syncfusion Angular Grid with Django REST Framework (Custom Binding) + Microsoft SQL Server

A clean, production‑oriented example showing how to connect **Syncfusion Angular Grid** to **Django REST Framework (DRF)** and **SQL Server** using **Custom Binding**. Supports server‑driven paging, sorting, filtering, searching, and full CRUD with a REST‑native contract (`GET` with query params + `POST/PUT/DELETE` for edits).
...
- **CORS Ready**: Seamless local Angular ↔ Django development

## Prerequisites

- **Node.js** LTS (v20+) with npm/yarn  
...
| `client/src/app/services/apiClient.ts` | Custom Binding request builder & CRUD |
| `client/src/app/app.component.ts` | Grid wired to DRF via Custom Binding |
| `client/src/styles.css` | Syncfusion Bootstrap 5.3 theme imports |
...
- Confirm `http://localhost:4200` in `CORS_ALLOWED_ORIGINS`
...
2. **Copy the GitHub URL**
    ...
    - Example : https://github.com/SyncfusionExamples/ej2-angular-grid-samples/tree/master/connecting-to-backends/syncfusion-angular-grid-custom-binding-with-django-server
